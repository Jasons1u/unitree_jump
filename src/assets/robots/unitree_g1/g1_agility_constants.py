"""Unitree G1 constants — Agility variant with custom waist gains.

Copy of ``g1_constants.py`` with softer proportional gains on the three waist
joints (damping unchanged):

    waist_yaw_joint   : Kp 35  (default 40.18), Kd 2.558 (DAMPING_7520_14)
    waist_pitch_joint : Kp 23  (default 28.50), Kd 1.814 (DAMPING_5020 * 2)
    waist_roll_joint  : Kp 23  (default 28.50), Kd 1.814 (DAMPING_5020 * 2)

``waist_yaw_joint`` is split out of the shared hip actuator group so its gain
can be set independently of the hips.
"""
...
# Custom waist proportional gains (Agility variant). Damping is left at the
# stock computed defaults (DAMPING_7520_14 for yaw, DAMPING_5020 * 2 for
# pitch/roll); only stiffness is overridden.
STIFFNESS_WAIST_YAW = 35.0
STIFFNESS_WAIST_PITCH_ROLL = 23.0
...
# waist_yaw_joint split out below into G1_ACTUATOR_WAIST_YAW for a custom gain.
...
# waist_yaw uses a 7520_14 motor, but with a custom (softer) stiffness while
# keeping the stock 7520_14 damping.
...
# Stiffness uses the custom Agility value; damping stays at the stock default.
G1_ACTUATOR_WAIST = BuiltinPositionActuatorCfg(
  target_names_expr=("waist_pitch_joint", "waist_roll_joint"),
  stiffness=STIFFNESS_WAIST_PITCH_ROLL,
...
G1_AGILITY_ARTICULATION = EntityArticulationInfoCfg(
  actuators=(
    G1_ACTUATOR_5020,
    G1_ACTUATOR_7520_14,
    G1_ACTUATOR_7520_22,
...
def get_g1_agility_robot_cfg() -> EntityCfg:
  """Get a fresh G1 Agility robot configuration instance.
  ...
  return EntityCfg(
    ...
    spec_fn=get_spec,
    articulation=G1_AGILITY_ARTICULATION,
  )


G1_AGILITY_ACTION_SCALE: dict[str, float] = {}
for a in G1_AGILITY_ARTICULATION.actuators:
  ...
  for n in names:
    G1_AGILITY_ACTION_SCALE[n] = 0.25 * e / s
...
if __name__ == "__main__":
  ...
  robot = Entity(get_g1_agility_robot_cfg())
